...
@onready var filepath_label: Label = $VBoxContainer/Panel/FilepathLabel

# SOFIE: For å gjøre vinduet dragable
var dragging: bool = false
var drag_offset: Vector2 = Vector2.ZERO

# SOFIE: For å hente image name for visning av filsti
var current_image_name: String = ""
...
func show_image(texture: Texture2D, image_name: String) -> void:
	if texture == null:
		push_error("Ingen texture sendt til ImagePopUp")
		return
	
	# SOFIE: Setter self variabler 
	image_rect.texture = texture
	current_image_name = image_name
	filepath_label.text = "filepath: /home/pictures/%s" % image_name
	
	# SOFIE: Bilde format
	var img_size: Vector2 = texture.get_size()
	
	# ikke lengre i bruk da bilder er satt til standardisert strls :D 
	...
	var panel_size := Vector2(700, 900) #Siden bildene er 600x800, gir margin 
...
# SOFIE: Tillater å exit med escape tastetrykk
...
func _on_img_panel_gui_input(event: InputEvent) -> void:
